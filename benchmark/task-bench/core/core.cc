                              ...
                              char *scratch_ptr, size_t scratch_bytes) const
{
  ...
  float selectivity = pow(2, point);
  printf("Executes timestep %ld, point %ld\n",timestep, point);
  ...
  // Validate input
  {
    ...
    for (size_t span = 0; span < num_deps; span++) {
	  ...
	  int expected_input_size = output_bytes_per_task/pow(2, dep);
	  printf("expected input bytes: %ld, inputs bytes %ld, span %ld, deps.first: %ld, deps.second %ld, dep: %ld\n",
			  expected_input_size, input_bytes[idx], span, deps[span].first, deps[span].second, dep);
          assert(input_bytes[idx] == expected_input_size); 
		  ...
		  //printf("task <%ld, %ld> recieves <%ld, %ld> at input, selectivity %f\n",timestep, point, input[i].first, input[i].second, selectivity);
#ifdef DEBUG_CORE
  ...
  // Validate output
  int expected_output_size = output_bytes_per_task/selectivity;
  printf("---------------------------------> task <%ld, %ld> with selectivity %f has output bytes %ld, expected output bytes %ld \n",timestep, point, selectivity, output_bytes, expected_output_size);
  assert(output_bytes == expected_output_size);
  ...
  printf("---------------------------------> task <%ld, %ld> with selectivity %f has output bytes %ld \n",timestep, point, selectivity, output_bytes);
